...
searchInput.addEventListener("input", () => {
  ...
  let matches = Object.keys(roadmaps).filter(role =>
    ...
    roadmaps[role].category === "software"
  );
  ...
  if (matches.length > 0) {
    ...
    noMatch.textContent = "No Software role found";
...
function showRoadmap(role) {
  ...
  if (!data || data.category !== "software") {
    roadmapDiv.style.display = "block";

    roadmapDiv.innerHTML = `<p style="color:red;">Not available in Software</p>`;
    return;
  }
